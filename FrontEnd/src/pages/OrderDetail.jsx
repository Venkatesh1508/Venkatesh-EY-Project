...
const OrderDetail = () => {
  ...
  const handlePlaceOrder = async () => {
    if (!user?.email){
      alert("Please Signin to place an order")
      return
    }
  ...
  return (
    <div className='row p-2 justify-content-center align-items-center'>
      <div>
        <h1 className='fs-1 text-dark mb-4 text-center'>Add Your Order</h1>
      </div>
      <div className="card p-3" style={{ width: "18rem" }}>
        ...
        <div className="card-body">
          ...
          <p className="card-text fs-3">Amount: ₹{dish.price}</p>
          <p className="card-text fs-3">Restaurant Email : {dish.hotelEmail}</p>
